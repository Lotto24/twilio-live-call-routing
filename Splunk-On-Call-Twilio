...
function handler (context, event, callback) {
  let {ALERT_HOST, API_HOST, NUMBER_OF_MENUS, voice, NO_VOICEMAIL, NO_CALL, GREETING, FALLBACK_NUMBER, FALLBACK_TITLE, GREETING_RECEIVE_CALL} = context;
  ...
  const messages = {
    ...
    fallbackTitle: _.isUndefined(FALLBACK_TITLE) ? "in case you want to reach business instead of on call" : FALLBACK_TITLE,
    ...
    pressKeyToConnect: _.isUndefined(GREETING_RECEIVE_CALL) ? 'This is Splunk Lyve Call Routing. Press any number to connect.' : GREETING_RECEIVE_CALL,
  ...
  context.fallbackNumber = _.isUndefined(FALLBACK_NUMBER)
    ? null
    : FALLBACK_NUMBER;
  context.headers = {
  ...
  return new Promise((resolve, reject) => {
    ...
    // Repeats the call or message menu if caller did not enter a valid response
      ...
      .then(response => {
        ...
        // that if selected is immediately redirected to the given number
        if (context.fallbackNumber != null) {
          teamsArray.push({
            name: "fallback_team",
...
function buildOnCallList (twiml, context, payload) {
  ...
  return new Promise((resolve, reject) => {
    ...
    // in case fallback was selected, just call the number
    if (teamsArray[0].slug == context.fallbackNumber) {
      log('calling fallback number', context.fallbackNumber);
      twiml.redirect(
        generateCallbackURI(
          context,
          {
            callerId,
            firstCall: true,
            phoneNumbers: [{ phone: context.fallbackNumber, user: teamsArray[0].title }],
            realCallerId,
            runFunction: 'call',
            teamsArray
          }
...
function postToVictorOps (event, context, payload) {
  return new Promise((resolve, reject) => {
    ...
    // in case fallback was selected, we skip posting to VictorOps
    if (teamsArray[0].slug == context.fallbackNumber) {
      log('calling fallback number completed; skipping post to VictorOps');
      return resolve('');
    }
